...
WRONG:
```ts
socket.on("selection-changed", (m) => { pool.setSelection(m.page, m.sel) })
// an out-of-order frame snaps the selection back to a stale element
```
RIGHT:
```ts
socket.on("selection-changed", (m: { page: string; sel: SelectionData; seq: number }) => {
  const cur = pool.lastSelection(m.page)
  if (cur && m.seq <= cur.seq) return          // stale — drop
  pool.setSelection(m.page, { ...m.sel, seq: m.seq })
})
```
