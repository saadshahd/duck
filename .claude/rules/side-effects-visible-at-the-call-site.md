...
WRONG:
```ts
function summarize(data: Data): Summary {
  logger.info('summarizing', data.content.length); // hidden effect inside pure logic
  return { total: [...preOrder(data)].length };
}
```
RIGHT:
```ts
...
logger.info('summarized', { total: summary.total }); // effect at the edge
```
