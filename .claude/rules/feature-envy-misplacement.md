...
```ts
// query.ts
function isEmptyHolder(node: OutlineNode, comp: ComponentData): boolean {
  return comp.props.id != null && slotKeysOf(comp).length === 0 && comp.type !== 'Root';
}
```
RIGHT:
```ts
// component.ts — the logic moves to the domain it's envious of
function isEmptyHolder(comp: ComponentData): boolean {
  return slotKeysOf(comp).length === 0 && comp.type !== 'Root';
}
```
...
Not-when: the function is an explicit adapter/mapper whose entire job is translating between two domains (e.g., a DTO mapper) — envy is expected there, it's the function's purpose, not a smell.
