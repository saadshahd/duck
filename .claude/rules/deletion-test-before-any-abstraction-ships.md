...
WRONG:
```ts
// exists because "we might swap tree libraries later"
function lookup(data: Data, id: string) { return findById(data, id) }
```
RIGHT:
```ts
// two storage adapters already exist (files, CMS); callers branch on neither — the interface is load-bearing today
function readData(page: string) { return activeStorage.readData(page) }
```
