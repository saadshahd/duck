...
WRONG:
```ts
add(data, component); // mutates data.content in place, no return
render(data);         // reader can't tell the tree changed
```
RIGHT:
```ts
const next = add(data, component); // derivation visible at the call site
render(next);
```
