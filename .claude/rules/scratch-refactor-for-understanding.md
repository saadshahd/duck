...
WRONG:
```ts
// spent 40 min renaming vars and extracting functions inside merge.ts to "get a feel for it"
// then just... kept committing that same diff as the actual fix
```
RIGHT:
```ts
// scratch/understand-merge.ts — exploratory only, deleted before commit
