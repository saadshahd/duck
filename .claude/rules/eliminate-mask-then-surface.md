...
async function parse(raw: string): Promise<Result<Data, ParseError>>
async function loadDraft(page: string): Promise<Result<Data, ParseError | StorageError>>
...
// caller must branch on NotFound and StorageError, so they surface; the
// fs-retry failure beneath is masked in the storage layer and never appears here
async function readData(page: string): Promise<Result<Data, NotFound | StorageError>>
```
