...
persistAndBroadcast(next); // what does this write? which page? does it hit the bridge?
```
RIGHT:
```ts
const next = applyOp(current, op);
storage.writeDraft(page, next); // applyOp = mechanism; the writes are visible policy
bridge.broadcast(page, next);
```
