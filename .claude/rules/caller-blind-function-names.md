...
WRONG:
```ts
function handleMorphButtonClick(entry: MorphEntry) {}
function buildOutlineForQueryTool(data: Data) {}
```
RIGHT:
```ts
function apply(entry: MorphEntry) {}
function outline(data: Data) {}
```
_Avoid_: names that embed a call site, an event, or a feature — `handle*Click`, `*ForQueryTool`, `*OnSubmit`, `*ForModal`.
