...
```ts
// old
export function applyOp(data: Data, op: Op) { /* ... */ }
// "seam" added as a parallel v2 the codebase migrates to gradually
export function applyOpV2(data: Data, op: Op, config: Config) { /* ... */ }
```
RIGHT:
```ts
...
export function applyOp(data: Data, op: Op, config: Config = defaultConfig) { /* ... */ }
```
