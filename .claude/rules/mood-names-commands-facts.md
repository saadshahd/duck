...
WRONG:
```ts
type Message = { type: 'SpecUpdate' } | { type: 'SelectionChange' };
```
RIGHT:
```ts
type Command = { type: 'PUSH_SPEC' } | { type: 'REQUEST_CAPTURE' };
type Fact = { type: 'SelectionChanged' } | { type: 'CaptureResponded' };
```
_Avoid_: tense-neutral message nouns (`SpecUpdate`, `SelectionChange`) that hide whether the thing is a request or a result.
