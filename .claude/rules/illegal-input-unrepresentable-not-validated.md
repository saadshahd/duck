...
WRONG:
```ts
const capture = (mode: string) => { /* runtime: if (mode !== 'viewport' && mode !== 'element' ...) */ }
capture('full') // compiles, fails at runtime — should be 'fullPage'
```
RIGHT:
```ts
type CaptureMode = 'viewport' | 'element' | 'fullPage'
const capture = (mode: CaptureMode) => { /* mode is already known valid */ }
capture('full') // will not compile
```
