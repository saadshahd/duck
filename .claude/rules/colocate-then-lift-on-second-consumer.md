...
WRONG:
```tsx
const InsertContext = createContext({ highlightedName: null as string | null, setHighlightedName: (v: string | null) => {} });
// consumed by exactly one <CatalogPicker>, three files away
```
RIGHT:
```tsx
