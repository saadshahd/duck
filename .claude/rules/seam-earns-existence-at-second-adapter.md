...
WRONG:
```ts
interface CaptureSink { save(image: string): Promise<void> }
class FileCaptureSink implements CaptureSink { /* only impl */ }
// "in case we store captures in S3 later"
```
RIGHT:
```ts
// Two adapters exist NOW: production FileStorage + test fake.
interface Storage { readData(page: string): Effect.Effect<Data, NotFound | StorageError> }
class FileStorage implements Storage { /* prod */ }
class FakeStorage implements Storage { /* test */ }
```
