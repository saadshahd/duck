...
WRONG:
```ts
// the morph picker the parent walks through, over shared selection, dressed up as an actor
pickerActor.send({ type: 'NEXT_VARIANT' });
overlay.pickerIndex = pickerActor.getSnapshot().index;   // parent mutating child internals
```
RIGHT:
```ts
// lifecycle bound to the parent → just nested state
type MorphPicker = { phase: 'closed' } | { phase: 'browsing' } | { phase: 'applied' };
// reserve the message-addressed reducer for the history actor: it self-resets and outlives any one selection
```
