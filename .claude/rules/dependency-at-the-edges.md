...
function makeComponent(type: string) {
  return { type, props: { id: Math.random().toString(36), createdAt: new Date() } };
}
```
RIGHT:
```ts
function makeComponent(type: string, deps: { clock: Clock; ids: Ids }) {
  return { type, props: { id: deps.ids.next(), createdAt: deps.clock.now() } };
}
```
