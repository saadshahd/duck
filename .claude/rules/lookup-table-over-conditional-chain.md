...
WRONG:
```ts
function summary(op: Op) {
  if (op.op === 'add') return 'Added';
  if (op.op === 'remove') return 'Removed';
  if (op.op === 'update') return 'Updated';
  if (op.op === 'move') return 'Moved';
}
```
RIGHT:
```ts
const summaries = {
  ...
  update: 'Updated',
  move: 'Moved',
} as const satisfies Record<Op['op'], string>;

const summary = (op: Op) => summaries[op.op];
```
