...
WRONG:
```ts
export function applyOps(ctx: McpContext, args: ApplyArgs) {
  // ...existing untested op loop...
  if (op.type === 'add' && op.slotPath.length === 0) {   // new behavior grown inline
    /* twenty lines validating root placement */
  }
}
```
RIGHT:
```ts
export function applyOps(ctx: McpContext, args: ApplyArgs) {
  // ...existing untested op loop, untouched...
  const rejection = RootPlacement.reject(op);  // sprouted, independently tested
}
```
