...
WRONG:
```ts
type Node = { props: { id: string } };
function selectElement(node: Node) { /* ... */ }
async function loadComponentRecord(id: string): Promise<Node> { /* ... */ }
```
RIGHT:
```ts
type ComponentData = { props: { id: string } };
function selectComponent(component: ComponentData) { /* ... */ }
async function loadComponent(id: string): Promise<ComponentData> { /* ... */ }
```
