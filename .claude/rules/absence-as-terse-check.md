...
WRONG:
```ts
if (element === null || element === undefined) return;              // mechanical enumeration
if (slotKeysOf(component).length === 0) return;                     // unnamed emptiness
function findById(id: string): Option<ComponentData> { /* ... */ }  // wrapper over native sugar
```
RIGHT:
```ts
if (!element) return;                          // truthiness is the default question
if (isEmpty(slotKeysOf(component))) return;    // one named predicate, defined ONCE in the codebase
const label = field.metadata?.label ?? 'anon'; // absorb a foreign nullable at the boundary
```
