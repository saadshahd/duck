...
function trace<T>(label: string, value: T): void { console.log(label, value); } // T never reappears — just `unknown`
```
RIGHT:
```ts
function firstOf<T>(items: [T, ...T[]]): T { return items[0]; }  // T links input to output
function getIn<T, K extends keyof T>(obj: T, key: K): T[K] { return obj[key]; }
