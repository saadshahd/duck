...
WRONG:
```ts
interface ControlMeta { commitMode: 'continuous' | 'discrete' }
const fieldControls: Record<Field['type'], ControlMeta> = {
  text: { commitMode: 'continuous' },
  radio: { commitMode: 'discrete' },
}; // keys ARE checked — but commitMode widens; typeof fieldControls.radio.commitMode is the whole union, not 'discrete'
```
RIGHT:
```ts
const fieldControls = {
  text: { commitMode: 'continuous' },
  radio: { commitMode: 'discrete' },
} as const satisfies Record<Field['type'], { commitMode: 'continuous' | 'discrete' }>;
// same key/shape check, values keep literal types: 'discrete', not the union
```
