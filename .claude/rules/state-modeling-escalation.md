...
const machine = createMachine({ states: { committed: { on: { EDIT: 'drafting' } }, drafting: {} } });
```
RIGHT:
```ts
// transition guarded and referenced from many call sites → map; illegal = no-op
const next: Record<DraftState, Partial<Record<DraftEvent, DraftState>>> = {
  committed: { APPLY: 'drafting' },
  drafting: { COMMIT: 'committed', DISCARD: 'committed' },
};
const advance = (s: DraftState, e: DraftEvent): DraftState => next[s][e] ?? s;
```
