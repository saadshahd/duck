...
function applyOp(op: Op, data: Data) {
  switch (op.op) {                    // closed, 4 fixed verbs — an interpreter buys nothing
    case 'add': return add(op, data);
    case 'update': return update(op, data);
    case 'remove': return remove(op, data);
    case 'move': return move(op, data);
  }
}
```
RIGHT:
```ts
type Control = { kind: 'radio' } | { kind: 'richtext' } | { kind: 'swatch' };
const controls: Record<Control['kind'], FieldControl> = { radio, richtext, swatch };
// add a control kind → the Record fails to compile until you supply its component
```
