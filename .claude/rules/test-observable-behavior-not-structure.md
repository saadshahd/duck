...
WRONG:
```ts
it('calls slotKeysOf while walking the tree', () => {
  const spy = spyOn(slotMod, 'slotKeysOf')
  outlineTree(data)
...
RIGHT:
```ts
it('collapses a stack below maxDepth to a childCount summary', () => {
  expect(outlineTree(data, 1)[0]).toEqual({ id: 'stack', type: 'Stack', childCount: 3 })
})
```
