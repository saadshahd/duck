...
describe("move — between slots", () => {
  it("moves from one slot to another", () => {
    const result = move(
      twoStacks(),
      "a",
      { at: "slot", parentId: "right", path: ["items"] },
      0,
    );
  ...
  it("moves from top-level into a slot", () => {
    ...
    const result = move(
      data,
      "solo",
      { at: "slot", parentId: "box", path: ["items"] },
      0,
    );
    const next = result._unsafeUnwrap();
...
describe("move — within slot", () => {
  it("reorders within the same slot", () => {
    const result = move(
      twoStacks(),
      "a",
      { at: "slot", parentId: "left", path: ["items"] },
      1,
    );
...
describe("move — errors", () => {
  ...
  it("circular-move: drop into self", () => {
    const result = move(
      nested(),
      "outer",
      { at: "slot", parentId: "outer", path: ["items"] },
      0,
    );
  ...
  it("circular-move: drop into descendant", () => {
    const result = move(
      nested(),
      "outer",
      { at: "slot", parentId: "inner", path: ["items"] },
      0,
    );
    expect(result._unsafeUnwrapErr().tag).toBe("circular-move");
  });
  ...
  it("circular-move: drop into deeper descendant", () => {
    ...
    const result = move(
      data,
      "a",
      { at: "slot", parentId: "c", path: ["items"] },
      0,
    );
    expect(result._unsafeUnwrapErr().tag).toBe("circular-move");
  });

  it("circular-move: drop into a descendant reachable only via an array-item slot", () => {
    ...
    const data: Data = {
      root: { props: {} },
      content: [card("outer", [{ content: [card("inner", [])] }])],
    };
    const result = move(
      data,
      "outer",
      { at: "slot", parentId: "inner", path: ["features", 0, "content"] },
      0,
    );
    expect(result._unsafeUnwrapErr().tag).toBe("circular-move");
  });
