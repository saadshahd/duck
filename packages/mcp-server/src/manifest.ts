...
const promptText = (config: Config): string => {
  const lines: string[] = [
    ...
    '- Slots are field keys of type: "slot". Children live at component.props.<slot> as an array; array-item slots live at component.props.<field>[<i>].<slot>.',
    '- Placement: root → parentId=null (omit slotPath). Slot → parentId=<id>, slotPath=["<slot>"] for a top-level slot or ["<field>", <i>, "<slot>"] for an array-item slot.',
    "",
    "## Components",
    "",
  ];
